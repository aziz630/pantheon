...
class AccountStudentsFeeController extends Controller
{
    ...
    {
        $data['allData'] = AccountStudentsFee::all();
        return view('pages.account.student_fee_view', $data);
    }
    ...
    public function Student_Fee_Add(){
    	...
    	return view('pages.account.student_fee_add',$data);
    ...
    public function Student_Fee_Get_Student(Request $request)
    {
        ...
        $html[$key]['tdsource']  .= '<td>'.$std['student']['std_name']. '<input type="hidden" name="session" value= " '.$std->academic_session_id.' " >'.'</td>';
    
        $html[$key]['tdsource']  .= '<td>'.$std['student']['std_father_name']. '<input type="hidden" name="class_id" value= " '.$std->class_id.' " >'.'</td>';
